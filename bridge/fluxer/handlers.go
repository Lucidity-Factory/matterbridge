...
func (b *Bfluxer) handleQuote(m *flo.Message, msg string) string {
	...
	if err != nil {
		b.Log.Errorf("Error getting quoted message %d:%d: %s", refMsgRef.ChannelID, refMsgRef.MessageID, err)
		return msg
	}
...
func (b *Bfluxer) messageCreate(e flo.MessageCreateEvent) { //nolint:funlen
	...
	if e.Content != "" {
		rmsg.Text = b.replaceChannelMentions(e.Content)
	}
...
func (b *Bfluxer) messageUpdate(e flo.MessageUpdateEvent) {
	if *e.GuildID != b.guildID {
...
func (b *Bfluxer) messageDelete(e flo.MessageDeleteEvent) {
	if *e.GuildID != b.guildID {
